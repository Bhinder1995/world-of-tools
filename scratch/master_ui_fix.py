...
def master_fix():
    ...
    for filepath in files:
        ...
        # 2. Handle Footer Injection/Update
        if '<footer' in html:
            # Replace existing footer regardless of content
            html = re.sub(r'<footer.*?>.*?</footer>', FOOTER_HTML, html, flags=re.DOTALL)
        else:
            # Inject before the first script tag that comes after <main> or just before </body>
            if '</main>' in html:
    ...
    print(f"Master Fix completed for {fixed_count} files.")
