...
def restore_tool(file_path):
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 4rem; align-items: center;">
                <div>
                    ...
                    <p style="font-size: 1.15rem; color: #4b5563; line-height: 1.6; margin-bottom: 2.5rem; font-weight: 500;">
                        WorldOfTools uses next-generation client-side processing. Your data <span style="color: #000; font-weight: 800;">NEVER leave your device</span>. Everything happens locally in your browser's memory.
                    </p>
                </div>
            </div>
        </div>
    ...
    target = soup.find('main') or soup.find('footer') or soup.find('body')
    if target and "Privacy Standard" not in content:
        trust_soup = BeautifulSoup(trust_block_html, 'html.parser')
        # If it's footer or body, insert BEFORE it. If main, append INSIDE it.
        if target.name == 'main':
            target.append(trust_soup)
        else:
            target.insert_before(trust_soup)
...
def main():
    ...
    exclude = ["index.html", "about-us.html", "contact-us.html", "privacy.html", "terms.html", "image-to-text-ocr.html"]
